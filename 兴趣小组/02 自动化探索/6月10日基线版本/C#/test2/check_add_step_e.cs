...
namespace test2
{
    public partial class check_add_step_e : Form
    {
        ...
        private void check_step_a_button_Click(object sender, EventArgs e)
        {
            ...
            if (str_check_name != "")
            {
                sql_max = " select max(check_order)  from check_page where check_point_name='" + str_check_name.Trim() + "';";
            ...
            if (this.dataGridView2.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < this.dataGridView2.Rows.Count; i++)
                {
                    string sql = @"insert  into check_page
                        (check_order,check_point_name,page_name,element_name,sysname) values(";
