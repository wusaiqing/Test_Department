...
namespace test2
{
    public partial class add_step : Form
    {
        ...
        public void query(string str1, string str2)
        {
            ...
            if (str1 != "")
            {
                sql = sql + "  and (sysname = '" + str1 + "' or sysname ='all_system')";
            }
            else
            {
                if (str2 != "")
                {
                    string sql_f = "select distinct sysname  from config  where  function_name like'%" + str2.Trim() + "%'";
                    DataTable dt = MySqlHelper.GetDataSet(MySqlHelper.Conn, CommandType.Text, sql_f, null).Tables[0];
                    if (dt != null)
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            sql = sql + "  and (sysname ='" + dt.Rows[i][0].ToString() + "' or sysname ='all_system')";
                        }
                    }
                }
            }
